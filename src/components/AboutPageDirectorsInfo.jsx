  ...
  return (
    <section className="py-140 bg-secondary-deep_jungle_green relative max-730:py-100 z-2">
      <img
        width={200}
        height={200}
        className="absolute -left-100 top-0 max-730:h-100 object-cover max-730:object-bottom"
        src={circleShape}
        alt="circle shape"
      />
      ...
      <div className="container">
        <h2 className="text-center max-w-917 mx-auto mb-16 max-940:mb-14 max-730:mb-12">
          Meet the directors
        </h2>
        <ul className="grid grid-cols-3 gap-30px gap-y-12 max-800:gap-5 max-730:grid-cols-2 max-730:gap-2.5 max-730:max-w-573 max-730:mx-auto max-540:grid-cols-1 max-540:gap-6">
          {directorsInfo.map((e, index) => {
            return (
              <li
                key={e.id}
                className="flex flex-col items-center relative bg-secondary-sacramento_state_green pt-8 pb-56px mb-7 text-center"
              >
                <img
                  width={96}
                  height={96}
                  ...
                  className="mb-4 rounded-full border-2 border-secondary-rapture_blue"
                />
                <h3 className="text-secondary-rapture_blue">{e.name}</h3>
                <p className="italic small-p font-[500!important]">{e.job}</p>
                <button
                  onClick={() => toggleDirectorState(index)}
                  className={`flex absolute items-center justify-center -bottom-7 rounded-full bg-primary-light_coral w-56 h-56 z-1 transition-colors transition-transform-opacity plus-btn hover:bg-secondary-rapture_blue ${
                    directorStates[index] ? "plus-btn_active" : ""
                  }`}
                >
                ...
                <div
                  className={`${
                    directorStates[index] ? "flex" : "hidden"
                  } absolute bg-secondary-sacramento_state_green top-0 right-0 left-0 bottom-0 flex-col py-9 px-12 w-full h-full items-center directors-card max-1050:px-10 max-1050:py-7 max-940:px-5 max-800:px-3 max-730:px-6 max-730:pt-9 max-730:pb-12 max-540:px-56px max-370:px-10`}
                >
                  <h3 className="text-secondary-rapture_blue mb-2">{e.name}</h3>
                  <p className="max-w-254 mb-6 small-p max-940:mb-4 max-800:mb-3 max-540:max-w-max max-540:mb-6">
                    {e.description}
                  </p>
                    ...
                    <li>
                      <a href={e.twitter} target="_blank">
                        <svg
                          ...
                          className="text-white hover:text-primary-light_coral transition-colors"
                          width="20"
                          height="24"
                          viewBox="0 0 24 20"
                          fill="none"
